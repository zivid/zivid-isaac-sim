...
from isaacsim.zivid.camera import SamplingMode, ZividCamera, ZividCameraModelName
from isaacsim.zivid.mounts import Mount
from isaacsim.zivid.utilities.fov import clear_fov, draw_fov
...
class NewCameraWindow:
    ...
    def _add_clicked(self):
        ...
        mount_prim = self._usd_context.get_stage().GetPrimAtPath(mount_path)
...
class CameraWidget:
    ...
    def _capture_button_clicked(self):
        ...
        if data.size != 0:
            assert data.dtype == np.uint8, "Data should be of type uint8"
            h, w, c = data.shape
            self._image_widget.prepare_draw(data.shape[1], data.shape[0])

            self._image_provider.set_data_array(data, [w, h, c])
            self._width_widget.set_value(h)
            self._height_widget.set_value(w)
    ...
    def _build_image_frame(self):
        image_frame = CollapsableFrame("RGB", collapsed=False)
        with image_frame:
            with omni.ui.VStack(style=get_style(), spacing=5, height=0):
                self._image_widget = omni.ui.ImageWithProvider(
                    self._image_provider, width=omni.ui.Length(300), height=omni.ui.Length(300)
                )
                self._width_widget = IntField(
                    "Width",
                    default_value=0,
    ...
    def _refresh_ui(self, camera: ZividCamera):
        self._camera = camera
        self._model_dropdown.set_selection(camera.get_camera_model().name.name)
