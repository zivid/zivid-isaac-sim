...
color_map = [carb.ColorRgba(cmap(i / len(modes))) for i in modes]

zivid_camera.set_sampling_mode(modes[mode_counter])  # Set the initial model
...
while simulation_app.is_running():
    ...
    if i > 0 and i % 20 == 0 and mode_counter < num_modes:
        if mode_counter == 0:
            draw.clear_points()
        print(f"Switched to mode: {modes[mode_counter].name}")
        ...
        for point in valid_points:
            ...
            colors.append(color_map[mode_counter])
            sizes.append(10.0)
        ...
        mode_counter += 1
        if mode_counter >= num_modes:
            mode_counter = 0

        zivid_camera.set_sampling_mode(modes[mode_counter])
